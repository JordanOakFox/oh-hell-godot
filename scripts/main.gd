...
func _build_ui() -> void:
	...
	join_button.text = "Join localhost"
...
func _publish_state() -> void:
	if multiplayer.multiplayer_peer and multiplayer.is_server():
		...
		_receive_private_state(_public_state(), state["hands"][my_seat], my_seat)
...
func _peer_for_seat(seat: int) -> int:
	...
	var index := seat - 1
	return peers[index] if index < peers.size() else 1
